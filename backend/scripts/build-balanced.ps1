# Build with Balanced Environment Script
# This script isolates Rust toolchains while keeping necessary Solana tools

Write-Host "Starting Build with Balanced Environment..." -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "Setting up balanced build environment..." -ForegroundColor Yellow
...
Write-Host ""
Write-Host "Building smart contract with balanced environment..." -ForegroundColor Yellow

# Change to smart contract directory
Set-Location "..\smart-contract"

# Set environment variables for balanced approach
$env:HOME = $env:USERPROFILE
$env:CARGO_HOME = "$env:USERPROFILE\.cargo"
$env:RUSTUP_HOME = "$env:USERPROFILE\.rustup"

# CRITICAL: Force system Rust toolchain but keep Solana CLI tools in PATH
$env:RUSTUP_TOOLCHAIN = "1.79.0"
$env:CARGO = "$env:USERPROFILE\.cargo\bin\cargo.exe"
$env:RUSTC = "$env:USERPROFILE\.cargo\bin\rustc.exe"

# CRITICAL: Keep original PATH but prioritize system Rust
$originalPath = $env:PATH
$systemRustPath = "$env:USERPROFILE\.cargo\bin;$env:USERPROFILE\.rustup\toolchains\1.79.0-x86_64-pc-windows-msvc\bin"
$env:PATH = "$systemRustPath;$originalPath"
...
Write-Host "Balanced PATH: System Rust first, then original PATH" -ForegroundColor Cyan
...
# CRITICAL: Try to build with explicit toolchain
