# Force Anchor to Use Solana Toolchain Script
# This script explicitly forces Anchor to use the Solana toolchain

Write-Host "Starting Force Solana Toolchain Build..." -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "Building smart contract with forced Solana toolchain..." -ForegroundColor Yellow
...
# CRITICAL: Force the use of Solana toolchain
$env:RUSTUP_TOOLCHAIN = "solana"
$env:CARGO = "$env:USERPROFILE\.rustup\toolchains\solana\bin\cargo.exe"
$env:RUSTC = "$env:USERPROFILE\.rustup\toolchains\solana\bin\rustc.exe"

Write-Host "Environment variables set" -ForegroundColor Cyan
Write-Host "Forcing Solana toolchain: $env:RUSTUP_TOOLCHAIN" -ForegroundColor Cyan
...
Write-Host "Current Rust version: $rustVersion" -ForegroundColor Cyan
...
# Try to build with Anchor using Solana toolchain
Write-Host "Attempting to build with Anchor using Solana toolchain..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "2. Check if the environment variables are properly set" -ForegroundColor White
    Write-Host "3. Try running: rustup run solana anchor build" -ForegroundColor White
    Write-Host "4. Check if the Solana toolchain has the required targets" -ForegroundColor White
}
