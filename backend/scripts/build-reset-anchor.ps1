# Build with Reset Anchor Configuration Script
# This script completely resets Anchor's configuration and forces it to use system Rust

Write-Host "Starting Build with Reset Anchor Configuration..." -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "Completely resetting Anchor configuration..." -ForegroundColor Yellow

# Check current Rust toolchains
Write-Host "Current Rust toolchains:" -ForegroundColor Cyan
rustup toolchain list

# Remove any problematic toolchains
Write-Host "Removing problematic toolchains..." -ForegroundColor Yellow
rustup toolchain uninstall solana 2>$null
rustup toolchain uninstall 1.75.0-x86_64-pc-windows-msvc 2>$null

# Set Rust 1.79.0 as default (this should be compatible)
Write-Host "Setting Rust 1.79.0 as default..." -ForegroundColor Yellow
rustup default 1.79.0

# CRITICAL: Clear any existing RUSTUP_TOOLCHAIN environment variable
if ($env:RUSTUP_TOOLCHAIN) {
    Write-Host "Removing RUSTUP_TOOLCHAIN environment variable: $env:RUSTUP_TOOLCHAIN" -ForegroundColor Yellow
...
# Install Solana targets for Rust 1.79.0
...
if ($targetCheck) {
    ...
    Write-Host "Trying to install with nightly..." -ForegroundColor Yellow
    rustup run nightly rustup target add bpfel-unknown-unknown
}
...
# CRITICAL: Force system Rust and clear any Anchor toolchain references
