# Build with rustup run solana Script
# This script uses rustup run solana to explicitly run Anchor with the Solana toolchain

Write-Host "Starting Build with rustup run solana..." -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "Building smart contract with rustup run solana..." -ForegroundColor Yellow
...
# Verify we're using the Solana toolchain
Write-Host "Verifying Solana toolchain..." -ForegroundColor Yellow
$rustVersion = rustup run solana rustc --version 2>&1
Write-Host "Solana toolchain Rust version: $rustVersion" -ForegroundColor Cyan

# Check if we can access Solana targets
Write-Host "Checking Solana targets..." -ForegroundColor Yellow
$targetCheck = rustup run solana rustc --print target-list 2>&1 | Select-String "bpfel-unknown-unknown"
if ($targetCheck) {
    ...
    Write-Host "This suggests the Solana toolchain is not properly configured" -ForegroundColor Red
    exit 1
}

# Try to build with Anchor using rustup run solana
Write-Host "Attempting to build with rustup run solana anchor build..." -ForegroundColor Yellow
$buildOutput = rustup run solana anchor build 2>&1
...
if ($LASTEXITCODE -eq 0) {
    ...
    if (Test-Path "target\deploy\guess5_escrow.so") {
        ...
        # Deploy the contract using rustup run solana
    ...
    Write-Host ""
    Write-Host "=== TROUBLESHOOTING ===" -ForegroundColor Yellow
    Write-Host "1. The rustup run solana approach should force the Solana toolchain" -ForegroundColor White
    Write-Host "2. Check if the Solana toolchain has the required targets" -ForegroundColor White
    Write-Host "3. Try running manually: rustup run solana anchor build" -ForegroundColor White
    Write-Host "4. Check if Anchor is compatible with the Solana toolchain" -ForegroundColor White
}
