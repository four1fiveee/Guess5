# Build with Solana Toolchain Script
# This script uses the Solana toolchain to build the program

Write-Host "Starting Build with Solana Toolchain..." -ForegroundColor Green
Write-Host ""
...
Write-Host ""
Write-Host "Setting up Solana toolchain..." -ForegroundColor Yellow
...
if ($solanaToolchain) {
    Write-Host "✅ Solana toolchain found: $solanaToolchain" -ForegroundColor Green
} else {
    Write-Host "❌ Solana toolchain not found. Installing..." -ForegroundColor Yellow
    
    # Try to install Solana toolchain
    Write-Host "Installing Solana toolchain..." -ForegroundColor Yellow
    rustup toolchain install solana
    
    if ($LASTEXITCODE -eq 0) {
        Write-Host "✅ Solana toolchain installed successfully" -ForegroundColor Green
    } else {
        Write-Host "❌ Failed to install Solana toolchain" -ForegroundColor Red
        Write-Host "Trying alternative approach..." -ForegroundColor Yellow
        
        # Try to install from Solana's installer
        Write-Host "Checking if Solana installer has Rust..." -ForegroundColor Yellow
        $solanaRustPath = "$env:USERPROFILE\.local\share\solana\install\releases\1.18.4\solana-release\bin\sdk\bpf\dependencies\platform-tools\rust"
        
        if (Test-Path $solanaRustPath) {
            Write-Host "✅ Found Solana Rust at: $solanaRustPath" -ForegroundColor Green
            $env:PATH = "$solanaRustPath\bin;$env:PATH"
            Write-Host "✅ Added Solana Rust to PATH" -ForegroundColor Green
        } else {
            Write-Host "❌ Solana Rust not found at expected location" -ForegroundColor Red
            Write-Host "Trying to find Solana Rust..." -ForegroundColor Yellow
            
            # Search for Solana Rust in common locations
            $possiblePaths = @(
                "$env:USERPROFILE\.local\share\solana\install\releases\*\solana-release\bin\sdk\bpf\dependencies\platform-tools\rust\bin\rustc.exe",
                "$env:USERPROFILE\.local\share\solana\install\releases\*\solana-release\bin\sdk\bpf\dependencies\platform-tools\rust\bin\rustc.exe"
            )
            ...
            foreach ($path in $possiblePaths) {
                ...
                if ($matches) {
                    $solanaRustDir = Split-Path (Split-Path $matches[0] -Parent) -Parent
                    Write-Host "✅ Found Solana Rust at: $solanaRustDir" -ForegroundColor Green
                    $env:PATH = "$solanaRustDir\bin;$env:PATH"
                    $env:CARGO = "$solanaRustDir\bin\cargo.exe"
                    $env:RUSTC = "$solanaRustDir\bin\rustc.exe"
                    $foundRust = $true
                    break
                }
            }
            ...
            if (-not $foundRust) {
                Write-Host "❌ Could not find Solana Rust" -ForegroundColor Red
                Write-Host "Please install Solana CLI first: https://docs.solana.com/cli/install-solana-cli-tools" -ForegroundColor Yellow
                exit 1
            }
        }
    }
...
Write-Host "Attempting to build with Anchor..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host ""
    Write-Host "=== TROUBLESHOOTING ===" -ForegroundColor Yellow
    Write-Host "1. The Solana toolchain should have resolved the target issue" -ForegroundColor White
    Write-Host "2. Check if Solana CLI is properly installed" -ForegroundColor White
    Write-Host "3. Try reinstalling Solana CLI: https://docs.solana.com/cli/install-solana-cli-tools" -ForegroundColor White
    Write-Host "4. Check if the Solana Rust binaries are accessible" -ForegroundColor White
}
