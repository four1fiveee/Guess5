...
# This script bypasses Anchor completely and builds directly with Cargo

Write-Host "Starting Guess5 Smart Contract Devnet Deployment with Direct Cargo..." -ForegroundColor Green
Write-Host ""
...
# CRITICAL: Force system Rust and bypass Anchor completely
Write-Host "Setting up system Rust for direct Cargo build..." -ForegroundColor Yellow
...
# CRITICAL: Override PATH to prioritize system Rust
Write-Host "Overriding PATH to prioritize system Rust..." -ForegroundColor Yellow
...
# CRITICAL: Build directly with Cargo instead of Anchor to avoid Cargo.lock recreation
Write-Host "Building smart contract directly with Cargo (bypassing Anchor)..." -ForegroundColor Yellow
...
if ($LASTEXITCODE -eq 0) {
    Write-Host "✅ Smart contract built successfully with Cargo!" -ForegroundColor Green
    ...
    # Deploy the contract using Anchor (but with our built binary)
    ...
    Write-Host "❌ Failed to build smart contract with Cargo" -ForegroundColor Red
    Write-Host ""
    Write-Host "=== TROUBLESHOOTING ===" -ForegroundColor Yellow
    Write-Host "1. The issue was Anchor recreating Cargo.lock files" -ForegroundColor White
    Write-Host "2. We've bypassed Anchor completely and built with Cargo directly" -ForegroundColor White
    Write-Host "3. If this still fails, try manually:" -ForegroundColor White
    Write-Host "   cd ..\smart-contract\programs\guess5-escrow" -ForegroundColor White
    Write-Host "   cargo build --release" -ForegroundColor White
    Write-Host ""
    Write-Host "4. The key insight: Bypass Anchor's Cargo.lock recreation!" -ForegroundColor Green
}
