# Simple PowerShell script for Guess5 Smart Contract Devnet Deployment
# This script focuses on the core issues without complex fallback strategies

Write-Host "Starting Simple Guess5 Smart Contract Devnet Deployment..." -ForegroundColor Green
Write-Host ""
...
# Install Rust nightly which supports Solana targets
Write-Host "Installing Rust nightly for Solana target support..." -ForegroundColor Yellow
rustup toolchain install nightly
rustup default nightly

# Install Solana targets
Write-Host "Installing Solana targets..." -ForegroundColor Yellow
rustup target add bpfel-unknown-unknown

# Verify the target is installed
Write-Host "Verifying Solana targets..." -ForegroundColor Yellow
$targetList = rustup target list --installed
if ($targetList -like "*bpfel-unknown-unknown*") {
    Write-Host "✅ Solana targets installed successfully" -ForegroundColor Green
} else {
    Write-Host "❌ Failed to install Solana targets" -ForegroundColor Red
    exit 1
}

# Try to build
Write-Host "Building smart contract with Rust nightly..." -ForegroundColor Yellow
anchor build
...
if ($LASTEXITCODE -eq 0) {
    ...
    Write-Host "1. The Cargo.lock file had version 4 which requires Rust nightly" -ForegroundColor White
    Write-Host "2. We've installed Rust nightly and Solana targets" -ForegroundColor White
    Write-Host "3. If this still fails, try manually:" -ForegroundColor White
    Write-Host "   cd ..\smart-contract" -ForegroundColor White
    Write-Host "   rustup default nightly" -ForegroundColor White
    Write-Host "   rustup target add bpfel-unknown-unknown" -ForegroundColor White
