 ...
 * Usage: npx ts-node backend/scripts/verify-match-payout.ts <matchId>
 */
...
// Import accounts using require to avoid TypeScript issues
const accounts = require('@sqds/multisig/lib/codegen/accounts');

const MATCH_ID = process.argv[2];

if (!MATCH_ID) {
  console.error('Usage: npx ts-node backend/scripts/verify-match-payout.ts <matchId>');
  process.exit(1);
}
  ...
  try {
    ...
    if (!transactionAccount) {
      ...
      try {
        ...
        const executed = (transaction as any).executed;
    ...
    for (const sig of winnerSignatures.slice(0, 5)) {
      try {
        ...
        if (tx && tx.meta && !tx.meta.err) {
          ...
          const winnerIndex = accountKeys.findIndex((key: any) => {
    ...
    for (const sig of feeSignatures.slice(0, 5)) {
      try {
        ...
        if (tx && tx.meta && !tx.meta.err) {
          ...
          const feeIndex = accountKeys.findIndex((key: any) => {
            const keyStr = key?.toString?.() || key?.toBase58?.() || String(key);
