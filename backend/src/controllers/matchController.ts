...
const generateReportHandler = async (req, res) => {
  try {
    ...
    // Get all matches with comprehensive data - using raw SQL to avoid TypeORM column issues
    ...
    // Generate CSV headers - comprehensive match data
    const csvHeaders = [
      'Match ID',
      'Player 1 Wallet',
      'Player 2 Wallet', 
    ...
    // Generate CSV rows - comprehensive match data
    const csvRows = matches.map(match => [
      ...
      match.player1PaymentConfirmed || false,
      match.player2PaymentConfirmed || false,
      match.winnerPayoutConfirmed || false,
      match.player1RefundConfirmed || false,
      match.player2RefundConfirmed || false,
      '', // 🔗 BLOCKCHAIN FEES (FROM PHANTOM) 🔗
