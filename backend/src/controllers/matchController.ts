...
// Import Escrow service for match settlement
...
const attemptAutoExecuteIfReady = async (
  ...
  try {
    ...
    // Check for escrow system only (Squads removed)
    ...
    // ESCROW SYSTEM: Escrow matches don't use proposals, so this function doesn't apply
  ...
  try {
    ...
    if (redisResult.status === 'matched' && redisResult.matchId) {
      ...
      return {
        ...
        // Squads fields removed - using escrow system only
...
const cleanupOldMatches = async (matchRepository: any, wallet: string) => {
  ...
  if (stalePaymentMatches.length > 0) {
    ...
    for (const match of stalePaymentMatches) {
      ...
      const hasEscrow = !!(match as any).escrowAddress;
      if ((match.player1Paid || match.player2Paid) || hasEscrow) {
...
const checkExistingMatches = async (matchRepository: any, wallet: string) => {
  ...
  if (existingMatches.length > 0) {
    ...
    return {
      status: existingMatch.escrowAddress ? 'matched' : 'vault_pending',
      matchId: existingMatch.id,
      player1: existingMatch.player1,
      player2: existingMatch.player2,
      entryFee: existingMatch.entryFee,
      // Squads fields removed - using escrow only
      vaultAddress: existingMatch.escrowAddress || null,
      ...
      if (bothPlayersHaveResults) {
        ...
        if (payoutResult && payoutResult.winner && payoutResult.winner !== 'tie') {
          ...
          if (!updatedMatch.squadsVaultAddress) {
            ...
            try {
              ...
              if (hasExistingProposal) {
                ...
                if ((updatedMatch as any).squadsVaultAddress) {
                  try {
                    // Create Squads proposal here
                    // This code path is for old Squads matches only
                  } catch (error: unknown) {
                    const errorMessage = error instanceof Error ? error.message : String(error);
            ...
            // Set fallback payment instructions (proposal will be created in background) - SQUADS ONLY
            if ((updatedMatch as any).squadsVaultAddress) {
            const paymentInstructions = {
              winner,
              loser,
        ...
        if (reloadedRows && reloadedRows.length > 0) {
          ...
          if (!(match as any).payoutProposalId && !(match as any).tieRefundProposalId && match.winner && (match as any).squadsVaultAddress) {
            ...
            try {
              ...
              // OLD SQUADS SYSTEM: Only process if this is a Squads match
              if ((match as any).squadsVaultAddress && match.winner !== 'tie') {
          ...
          if (retryLockAcquired) {
            ...
            try {
              ...
              if (checkRows && checkRows.length > 0 && (checkRows[0].payoutProposalId || checkRows[0].tieRefundProposalId)) {
                ...
                // OLD SQUADS SYSTEM: Only process if this is a Squads match
                if ((freshMatch as any).squadsVaultAddress) {
                  // OLD SQUADS SYSTEM: Use Squads proposal
                  const { PublicKey } = require('@solana/web3.js');
...
const confirmPaymentHandler = async (req: any, res: any) => {
  try {
    ...
    // ESCROW SYSTEM: Check for escrow address first
    if ((match as any).escrowAddress) {
      return res.status(400).json({ 
        error: 'This match uses the escrow system. Please use the escrow deposit endpoint.',
        escrowAddress: (match as any).escrowAddress
      });
    }
...
const manualExecuteProposalHandler = async (req: any, res: any) => {
  try {
    ...
    // ESCROW SYSTEM: Use settleMatch instead of executeProposal
    if ((match as any).escrowAddress) {
      return res.status(400).json({ 
