...
export const initializeDatabase = async () => {
  ...
  const runPreInitializationSchemaFixes = async () => {
    ...
    try {
      ...
      console.log('🔌 Running minimal pre-initialization checks...');
      await client.connect();
      
      // Only ensure critical tables exist - skip complex schema fixes to prevent hangs
      await ensureUserTable(client);
      console.log('✅ Minimal pre-initialization complete');
    } catch (error) {
      console.warn('⚠️ Pre-initialization failed (continuing):', error);
    } finally {
      if (client) {
        try {
          await client.end();
  ...
  while (retryCount < maxRetries) {
    try {
      ...
      // Minimal post-initialization checks
      console.log('🔍 Running minimal post-initialization checks...');
