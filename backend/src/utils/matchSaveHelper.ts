...
export async function saveMatchAndTriggerProposals(
  ...
  if (savedMatch.isCompleted && !wasCompleted) {
    enhancedLogger.info('≡ƒÄ» Match completed, triggering proposal creation via save helper', {
    ...
    onMatchCompleted(savedMatch).catch((error) => {
      enhancedLogger.error('Γ¥î Failed to auto-create proposals via save helper', {
        matchId: savedMatch.id,
